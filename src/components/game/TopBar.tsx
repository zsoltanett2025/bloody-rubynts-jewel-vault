...
export function TopBar(props: {
  ...
  /**
   * Ha a TopBar fixed, akkor a tartalom könnyen takarásba kerül.
   * Ez a spacer (alatta) segít, hogy ne lógjon rá.
   * Ha nálad már van külön “top padding” App-ban, nyugodtan hagyd false-on.
   */
  withSpacer?: boolean;
}) {
  const {
  ...
  // Safe-area (iOS notch) – így nem csúszik be a status bar alá
  const safeTop = "env(safe-area-inset-top, 0px)";

  return (
    <>
      {/* FIXED TOPBAR */}
      <div className="fixed top-0 left-0 right-0 z-[9999]" style={{ paddingTop: safeTop }}>
        {isTrial && (
        ...
        <div className="w-full px-2 sm:px-4 py-2 sm:py-3 bg-black/55 backdrop-blur-md border-b border-white/10 overflow-hidden">
          ...
          {/* MOBILE (below sm): compact score bar */}
      ...
      {/* SPACER: hogy ne takarja a fixed TopBar a tartalmat */}
      {withSpacer && (
        <div
          aria-hidden="true"
          style={{ paddingTop: safeTop }}
          className={[
            // kb. a TopBar magassága:
            "h-[68px] sm:h-[76px]",
            // ha a trial mobil figyelmeztetés aktív, adunk hozzá még egy sort:
            isTrial ? "md:h-[76px]" : "",
          ].join(" ")}
        />
      )}
